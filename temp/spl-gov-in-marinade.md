...
and a `governance_seed`. Previously, `governance_seed` was a pubkey of governed program, but this concept is obsolete now.
...
The voting population of the `council` or `community` may vote on the proposal. They can vote for the proposal or cast `deny` votes against it.
...
The `FinalizeVote` instruction checks the number of `yes` votes and the number of `deny` votes at all options of the proposal and considers the
