...
describe("solana-runtime-limitations", () => {
  ...
  it("transaction reallocate call", async () => {
    ...
    const reallocateAccount =
      await program.account.dataSizeReallocateAccount.fetch(
        accountKey.publicKey
      );
  ...
  it("cu limit", async () => {
    const tx = await program.methods.cuLimitInit(new BN(10000)).rpc();
...
async function executeTx({
  ...
  try {
    ...
    if (txPending) {
      ...
      console.debug(
        "Transaction failed at client typescript side or in simulation mode. Instruction programs:"
      );
      tx.instructions.forEach((value, index) =>
        console.debug(
          `[${index}]:${(value as TransactionInstruction).programId.toBase58()}`
        )
      );
    }
    throw e;
